-- ============================================================================
-- schema_snapshot_2026-09-19.sql
-- ----------------------------------------------------------------------------
-- Instantané complet du schéma Postgres de Cap Huma (projet Supabase),
-- généré le 19/09/2026, à partir du résultat collé par l'utilisateur de la
-- requête combinée unique de sql/GUIDE_INSTANTANE_SCHEMA.md (extraite
-- directement depuis l'éditeur SQL Supabase).
--
-- Remplace schema_snapshot_2026-09-12.sql comme référence à jour. L'ancien
-- fichier reste dans l'historique Git, pas besoin de le supprimer.
--
-- Résumé : 11 tables (inchangé), RLS activé sur les 11 (inchangé), 37
-- policies RLS (inchangé), 11 fonctions applicatives dans le schéma public
-- (+1 depuis le 12/09 : enforce_missions_occupant_staff_type — voir aussi
-- sql/staff_national_detachement_etape4_garde_fou_occupant.sql) dont 8
-- SECURITY DEFINER (is_admin, get_shared_talent, admin_revoke_user_sessions,
-- check_and_record_rate_limit, enforce_missions_occupant_staff_type,
-- audit_missions_changes, audit_share_tokens_changes, audit_talents_changes)
-- et 3 fonctions RPC "métier" STABLE non-DEFINER (get_notification_alerts,
-- get_pool_mission_counts, get_pool_talent_stats) ; 11 triggers (+2 depuis
-- le 12/09 : trg_enforce_missions_occupant_staff_type sur missions, BEFORE
-- INSERT/UPDATE) ; 50 index (+2 : idx_talents_tracking_pool et
-- idx_talents_tracking_pool_is_valid).
--
-- Écarts constatés par rapport au 12/09/2026 — voir détail en §8 :
--   1. missions_candidate_type_check autorise désormais 'detache' (résout
--      le point ouvert du check sécu staff national/détachement du 19/09 :
--      la contrainte avait déjà été élargie en base, seul le snapshot
--      versionné n'avait pas été repris depuis).
--   2. Nouvelle contrainte talents_staff_type_check ('expat'|'national') et
--      missions_contract_end_type_check ('date'|'cdi'|'ongoing').
--   3. missions passe de 23 à 26 colonnes, talents de 58 à 62 — les 7
--      nouvelles colonnes du chantier "Staff national et détachement"
--      (déjà connues, RAS).
--   4. Extension Postgres "unaccent" détectée dans le schéma public (4
--      fonctions : unaccent, unaccent(regdictionary,text), unaccent_init,
--      unaccent_lexize) — non documentée dans les instantanés précédents.
--      Code standard PostgreSQL (contrib), non versionné ci-dessous ; sans
--      lien apparent avec ce chantier.
--   5. Commentaire de travail oublié dans le corps de audit_talents_changes()
--      (visible en §6) — cosmétique, sans impact fonctionnel ou sécurité.
-- ============================================================================


-- ============================================================================
-- 1. TABLES ET COLONNES (information_schema.columns, 19/09/2026)
-- ============================================================================
...
-- ---- missions (26 colonnes -- +3 depuis le 12/09 : contract_end_type, detachment_source_mission_id, country_code) --------------------------------------------------------------
--   id                            uuid                      NOT NULL  default gen_random_uuid()
--   title                         text                      NOT NULL
...
--   contract_end_type             text                      NOT NULL  default 'date'  -- CHECK: 'date' | 'cdi' | 'ongoing'
--   detachment_source_mission_id  uuid                      NULL        -- FK -> missions(id) -- auto-référence, renseignée quand pool_level=project et candidate_type=detache
...
-- ---- rate_limit_log (4 colonnes -- déjà versionnée intégralement dans create_rate_limit_log.sql, non répétée ici en détail) --------------------------------------------------------
--   id             bigint                    NOT NULL
--   user_id        uuid                      NOT NULL
...
-- ---- talents (62 colonnes -- la plus grande table, cœur métier du site -- +4 depuis le 12/09 : staff_type, national_inactive_since, nationality_code, tracking_pool) ---------------------------------------------------------------
--   id                                      uuid                      NOT NULL  default gen_random_uuid()
...
--   staff_type                              text                      NOT NULL  default 'expat'  -- CHECK: 'expat' | 'national'
--   national_inactive_since                 date                      NULL
--   nationality_code                        text                      NULL
--   tracking_pool                           text                      NULL        -- FK -> pools(pool_id) -- pool de suivi post-inactivité (staff national), indexé (idx_talents_tracking_pool)
...
-- ============================================================================
-- 2. CONTRAINTES (PK / FK / UNIQUE / CHECK) — information_schema + pg_constraint, 19/09/2026
-- ----------------------------------------------------------------------------
...
--                   FK(detachment_source_mission_id) -> missions(id)  -- nouveau depuis le 12/09
...
--                   CHECK missions_candidate_type_check : candidate_type IN ('expat','nat','detache')
--                     -- 'detache' désormais présent (absent du texte capturé le 12/09 ; la
--                     -- contrainte avait déjà été élargie en base à cette date, seul le
--                     -- snapshot versionné n'avait pas été repris depuis)
--                   CHECK missions_pool_level_check : pool_level IN ('mission','project')
--                   CHECK missions_contract_end_type_check : contract_end_type IN ('date','cdi','ongoing')
--                     -- nouveau depuis le 12/09
...
--                   FK(created_by) -> users(id) ; FK(tracking_pool) -> pools(pool_id)  -- nouveau
...
--                   CHECK talents_staff_type_check : staff_type IN ('expat','national')  -- nouveau
...
-- ============================================================================
-- 3. ROW LEVEL SECURITY — activé/désactivé par table (pg_class, 19/09/2026)
-- ============================================================================
-- Les 11 tables ont RLS ACTIVÉ, sans exception, inchangé depuis le 12/09/2026 :
...
-- Ce chantier (staff national et détachement) n'a créé aucune table ni
-- modifié aucune policy — seuls un trigger et des fonctions RPC existantes
-- ont été touchés (§6/§7).


-- ============================================================================
-- 4. POLICIES RLS — résumé par table (pg_policies, 19/09/2026)
-- ----------------------------------------------------------------------------
-- Recontrôlé ce jour : toujours 37 policies au total, même répartition
-- qu'au 12/09/2026, aucun changement. Résumé en langage clair ; texte SQL
-- brut complet disponible via la requête #3 du guide si besoin de le
-- reproduire mot pour mot.
-- ============================================================================
...
--   (le garde-fou staff national/détachement est appliqué par le trigger du
--   §6/§7, pas par une policy — la RLS n'a pas bougé pour ce chantier)
...
-- ============================================================================
-- 5. GRANT — autorisations par table et par rôle (role_table_grants, 19/09/2026)
-- ----------------------------------------------------------------------------
-- Recontrôlé ce jour, inchangé depuis le 12/09/2026 : postgres et
...
-- ============================================================================
-- 6. FONCTIONS DU SCHÉMA PUBLIC (pg_proc, 19/09/2026) — code source complet
-- ----------------------------------------------------------------------------
-- 11 fonctions applicatives (+1 depuis le 12/09 : enforce_missions_occupant_
-- staff_type, déjà versionnée séparément dans
-- sql/staff_national_detachement_etape4_garde_fou_occupant.sql — reproduite
-- ici à l'identique pour la photo complète). get_shared_talent a aussi été
-- modifiée (filtre candidate_type='detache', voir
-- sql/staff_national_detachement_etape5_get_shared_talent_detachement.sql).
--
-- Non reproduites ici : les 4 fonctions de l'extension "unaccent"
-- (unaccent, unaccent(regdictionary,text), unaccent_init, unaccent_lexize)
-- détectées dans le schéma public — code standard PostgreSQL (contrib), pas
-- du code applicatif, sans lien identifié avec ce chantier.
--
-- Note : le corps de audit_talents_changes() ci-dessous contient un
-- commentaire de travail oublié ("-- seule ligne changée") sur la branche
-- DELETE — cosmétique, sans impact fonctionnel ni sécurité, mais à nettoyer
-- à l'occasion (CREATE OR REPLACE FUNCTION pour le retirer).
-- ============================================================================
...
AS $function$
  ...
  -- Talents actifs à risque de dévalidation (≥ 20 mois sans mission)
  select
    'at_risk'::text,
    t.pool,
    null::int,
...
-- ============================================================================
-- 7. TRIGGERS (information_schema.triggers, 19/09/2026)
-- ----------------------------------------------------------------------------
-- 11 triggers (+2 depuis le 12/09) = missions (5 : trg_audit_missions sur
-- INSERT/UPDATE/DELETE + trg_enforce_missions_occupant_staff_type sur
-- INSERT/UPDATE, BEFORE) + share_tokens (3) + talents (3), chacun appelant
-- sa fonction du §6.
-- ============================================================================

-- missions      -> trg_audit_missions                          (AFTER  INSERT/UPDATE/DELETE) -> audit_missions_changes()
-- missions      -> trg_enforce_missions_occupant_staff_type     (BEFORE INSERT/UPDATE)         -> enforce_missions_occupant_staff_type()  -- nouveau
...
-- ============================================================================
-- 8. NOTE — écarts constatés par rapport à l'instantané du 12/09/2026
-- ----------------------------------------------------------------------------
-- Résumé déjà donné en tête de fichier ; détail :
--
-- 1) missions_candidate_type_check autorise maintenant 'detache' (§2). Ceci
--    répond au point resté ouvert lors du check sécu du 19/09/2026 sur la
--    fonctionnalité "Staff national et détachement" : le texte du CHECK
--    capturé le 12/09 ne montrait que 'expat'/'nat', alors que le trigger
--    enforce_missions_occupant_staff_type (déjà en place au 12/09) et les
--    fonctions RPC manipulaient déjà 'detache'. La requête d'aujourd'hui
--    confirme que la contrainte avait déjà été élargie en base à cette
--    date-là — seul le fichier snapshot versionné était resté en retard
--    d'une mise à jour. Aucune action corrective nécessaire.
--
-- 2) Nouvelles colonnes/contraintes/index conformes au plan du chantier
--    "Staff national et détachement" (missions : contract_end_type,
--    detachment_source_mission_id, country_code ; talents : staff_type,
--    national_inactive_since, nationality_code, tracking_pool) — RAS.
--
-- 3) Extension "unaccent" présente dans le schéma public (§6) : ni signalée
--    ni expliquée dans les instantanés précédents. Extension standard de
--    PostgreSQL (recherche insensible aux accents), pas un risque en soi —
--    à mentionner pour mémoire seulement si vous ne l'avez pas installée
--    vous-même consciemment, pour confirmer qu'elle est attendue.
--
-- 4) Commentaire de travail oublié dans audit_talents_changes() (§6),
--    cosmétique.
-- ============================================================================


-- ============================================================================
-- 9. INDEX (pg_indexes, 19/09/2026) — 50 index au total sur les 11 tables (+2)
-- ============================================================================
...
--   idx_talents_tracking_pool (btree(tracking_pool))                    -- nouveau
--   idx_talents_tracking_pool_is_valid (btree(tracking_pool, is_valid)) -- nouveau
